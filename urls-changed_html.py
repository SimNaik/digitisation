...
# Ensure that the session state is set with correct directory paths for JSON and Image
if "processed_json_dir" in st.session_state and "processed_image_dir" in st.session_state:
    ...
    # Let the user input the directories if they are not in session state
    json_dir = st.sidebar.text_input("📂 JSON Directory")
    image_dir = st.sidebar.text_input("🖼 Image Directory")

    # Check if directories exist and set session state
    if json_dir and image_dir:
        if not Path(json_dir).is_dir():
            ...
            # Correctly set processed_folder to the parent directory of json_dir (i.e., /hesc104)
            st.session_state.processed_folder = str(Path(json_dir).parent)  # This will strip off '/json'

            st.sidebar.info(f"Processed folder set to: {st.session_state.processed_folder}")  # Confirm the processed folder path

# --------------- Function for URL Conversion -------------------
# Function to convert the URLs in JSON to HTML format
def convert_url_to_html(json_data):
    # Check if json_data is a list or a dictionary
    if isinstance(json_data, list):
        # If it's a list, loop through each page in the list
        for page in json_data:
            if "lines" in page:
                for line in page["lines"]:
                    if "text" in line:
                        # Find URLs and replace them with the HTML figure format
                        text = line["text"]
                        urls = re.findall(r'!\[\]\((https?://[^\)]+)\)', text)
                        for url in urls:
                            # Create the HTML format for each URL
                            html_format = f'<figure><img src="{url}" alt=""><figcaption></figcaption></figure>'
                            text = text.replace(f"![]({url})", html_format)
                        line["text"] = text
    elif isinstance(json_data, dict):
        # If it's a single dictionary (not a list), handle it accordingly
...
# Make sure 'selected_json' is defined before the URL conversion logic
if "selected_json" not in st.session_state:
    st.session_state.selected_json = None  # Default value if not set

# Sidebar controls for selecting directories and handling files
with st.sidebar:
    # Add the checkbox for converting URLs to HTML format
    convert_urls_to_html = st.checkbox("Change URLs to HTML format", value=False)

# Handle JSON directory and file loading
if "processed_json_dir" in st.session_state:
    ...
    # Add a selectbox for JSON file selection
    selected_json = st.selectbox("", options=sorted_json_files, index=st.session_state.current_json_idx)

    # Update session state for the selected JSON file
    st.session_state.selected_json = selected_json

    # Add the URL conversion logic after ensuring selected_json is defined
    if convert_urls_to_html and st.session_state.selected_json:
        ...
        # Save the updated JSON with URLs converted to HTML
        with open(json_path, 'w', encoding="utf-8") as f:
...
def merge_mmd_files():
    ...
    # Handle missing processed folder
    if not pdf_folder:
    ...
    # Correctly define the directory where per-page MMD files are stored
    # Ensure temp_mmd is searched directly under the pdf_folder (not under pdf_folder/json)
    mmd_dir = os.path.join(pdf_folder, "temp_mmd")
    
    # Handle missing temp_mmd directory
    ...
    # Merge the contents of the sorted MMD files
    merged_text = ""
    ...
    final_name = os.path.basename(pdf_folder)  # Use the folder name, e.g., "hesc104"
...
if json_dir:
    ...
    # Navigation buttons for previous and next JSON files
