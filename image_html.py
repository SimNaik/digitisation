...
# ------------------ Convert URLs to <figure> HTML format ------------------
def convert_urls_to_figure(json_data):
    # Regular expression to match URLs within '![]()'
    url_pattern = r'!\[\]\((https://cdn.mathpix.com/cropped/[^\)]+)\)'
    
    # Iterate through lines and check for URLs in the "text" field
    for line in json_data.get("lines", []):
        if "text" in line:
            ...
            for url in urls:
                # Replace the URL with the <figure> HTML structure
                new_text = f'<figure>\n  <img src="{url}" alt="">\n  <figcaption></figcaption>\n</figure>'
                line["text"] = text.replace(f'![]({url})', new_text)
...
if selected_json:
    ...
    # Convert URLs in the "text" field to <figure> HTML format
    page_data = convert_urls_to_figure(page_data)

    # Save the updated JSON data with converted URLs
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(page_data, f, indent=4, ensure_ascii=False)

    st.success("URLs have been converted to <figure> HTML format!")
